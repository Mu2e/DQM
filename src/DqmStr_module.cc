...
namespace mu2e {
  ...
  public:
    ...
    struct Config 
    {
      ...
      fhicl::Atom<art::InputTag> digiTag{Name("digiTag"),
          Comment("StrawDigi Collection"), art::InputTag()};
      fhicl::Atom<art::InputTag> adcTag{Name("adcTag"),
          Comment("ADCWaveform Collection"), art::InputTag()};
      //fhicl::Atom<art::InputTag> hitTag{Name("hitTag"),
      //   Comment("StrawHit Collection"), art::InputTag()};
      fhicl::Atom<art::InputTag> flagTag{Name("flagTag"),
          Comment("StrawHitFlag Collection"), art::InputTag()};
    };
  ...
  DqmStr::DqmStr(const Parameters& conf) :
    art::EDAnalyzer(conf),
    _conf(conf())
  {
  ...
  void DqmStr::beginJob(){
    ...
    if(!_conf.digiTag().empty()) {
      ...
      _hdDI = tfs->make<TH1D>( "DPlanePanel", "Digi Plane*6+Panel Index",240, -0.5, 239.5);
    ...
    //  _hhDI = tfs->make<TH1D>( "HPlanePanel", "Hit Plane*6+Panel Index",240, -0.5, 239.5);
    //  _hhSI = tfs->make<TH1D>( "HStraw", "Hit Straw Index",96, -0.5, 95.5);
    //}
  ...
  void DqmStr::analyze(const art::Event& event) {
    ...
    if(!_conf.adcTag().empty()) {
      auto adcH = event.getValidHandle<StrawDigiADCWaveformCollection>(_conf.adcTag());
