 ...
 public:
  struct Config {
    ...
    fhicl::Atom<art::InputTag> digiTag{
        Name("digiTag"), Comment("CaloDigi Collection"), art::InputTag()};
    fhicl::Atom<art::InputTag> clusterTag{
        Name("clusterTag"), Comment("CaloCluster Collection"), art::InputTag()};
  };
...
DqmCal::DqmCal(const Parameters& conf) : art::EDAnalyzer(conf), _conf(conf()) {
