...
namespace mu2e {
  ...
  public:
    ...
    struct Config 
    {
      ...
      fhicl::Atom<art::InputTag> digiTag{Name("digiTag"),
          Comment("CrvDigi Collection"), art::InputTag()};
      fhicl::Atom<art::InputTag> recoTag{Name("recoTag"),
          Comment("CrvRecoPulses Collection"), art::InputTag()};
      fhicl::Atom<art::InputTag> clusTag{Name("clusTag"),
          Comment("CrvCoincidenceClusters Collection"), art::InputTag()};
    };
  ...
  DqmCrv::DqmCrv(const Parameters& conf) :
    art::EDAnalyzer(conf),
    _conf(conf())
  {
  ...
  void DqmCrv::beginJob(){
    ...
    if(!_conf.recoTag().empty()) {
      ...
      _hrPH = tfs->make<TH1D>( "PEHeight", "PE from Pulse Height", 100, 0.0, 400.0);
      ...
      _hrLChi2 = tfs->make<TH1D>( "logchi2", "log10 Pulse fit chi2",100, -3.0, 5.0);
      _hrLE = tfs->make<TH1D>( "LeadingTime", "Leading Edge Time",100, 0.0, 2000.0);
      _hrLE2 = tfs->make<TH1D>( "LeadingTime2", "Leading Edge Time",100, 0.0, 100.0e3);
    }
  ...
  void DqmCrv::analyze(const art::Event& event) {
    ...
    if(!_conf.clusTag().empty()) {
      auto clusH = event.getValidHandle<CrvCoincidenceClusterCollection>(_conf.clusTag());
