...
int mu2e::DqmTool::commitValue() {
  ...
  command = "select iid from dqm.intervals where sid=" + std::to_string(sid) +
            ...
            " and start_time='" + time.startTime() + "'" + " and end_time='" +
            time.endTime() + "';";
  if (_verbose > 4) {
  ...
  if (!result.empty()) {  // found it
    ...
    command =
        ...
        "VALUES (" +
        std::to_string(sid) + "," + std::to_string(time.iov().startRun()) +
        "," + std::to_string(time.iov().startSubrun()) + "," +
        std::to_string(time.iov().endRun()) + "," +
        std::to_string(time.iov().endSubrun()) + ",'" + time.startTime() +
  ...
  for (auto const& value : valueColl) {
    ...
    command =
        ...
        std::to_string(sid) + "," + std::to_string(iid) + "," +
        std::to_string(vid) + ",'" + value.valueStr() + "','" +
        value.sigmaStr() + "'," + std::to_string(value.code()) +
        ") RETURNING nid;";
...
int mu2e::DqmTool::parseSource(DqmSource& source, std::string& runs,
  ...
  if (vs.size() == 6) {  // assume a file name
    ...
    if (cc.size() != 3) {
      std::cout << "ERROR - interpreting source as a file name, \n"
                << "      did not find 3 fields in the config field: " << vs[3]
                << std::endl;
      return 1;
    }
    process = cc[0];
    ...
    std::cout << "ERROR - source did not have 6 fields (for a file name)"
              << std::endl;
    std::cout << "           or 4 fields (for a parameter listing)"
              << std::endl;
    return 1;
  }
...
int mu2e::DqmTool::parseValue(DqmValue& rvalue, const std::string& vv) {
  ...
  if (vs.size() != 6) {
    std::cout << "ERROR - not 6 items in value statement: " << vv << std::endl;
    return 1;
  }
...
mu2e::DqmTool::StringVec mu2e::DqmTool::splitString(const std::string& command,
                                                    char del) {
...
int mu2e::DqmTool::parseAction() {
  ...
  while (ipt < _actionArgs.size()) {
    if (_actionArgs[ipt].substr(0, 2) != "--") {
      std::cout << "ERROR - action arguments did not start with a qualifier"
                << std::endl;
      return 1;
    }
  ...
  if (_action == "" || _action == "help") {
    ...
    std::cout
        << " \n"
           ...
           "    dqmTool print-numbers --source "
           "ntd.mu2e.DQM_strmA.test_file_000.0_0.root\n"
    ...
    std::cout
        << " \n"
           ...
           "    --runs STRING : run period like \"1100:0-1101:999999\" "
           "(optional) \n"
           "    --start STRING : start of DQM interval, like "
           "\"2022-01-01T14:00:00\" (optional) \n"
           "    --end STRING : end of DQM interval, like "
           "\"2022-01-01T14:00:00\" (optional) \n"
           "    --value STRING : either a csv string like "
